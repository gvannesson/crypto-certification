...
def build_document():
    ...
    doc.add_paragraph(
        ...
        "cadrage formel préalable avec des utilisateurs finaux : une limite assumée, cohérente "
        "avec un projet de certification individuel."
    )
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["3", "Graphiques", "Sélection paire + granularité, puis historique OHLCV et prédictions superposés (dashboard/charts.html)"],
            ["4", "Classification à la demande", "Sélection paire et granularité, appel réel à ml-api, résultat affiché (forecast/classify.html)"],
        ],
        col_widths=[2, 5, 10],
    )
    ...
    doc.add_paragraph(
        "Sur les cinq critères WCAG examinés en détail, deux sont clairement tenus : la "
        "structure sémantique des pages (balises <nav>/<main>, formulaires Django avec labels "
        "associés) et l'identification des erreurs de formulaire au bon endroit (LoginForm, "
        "RegisterForm). Le contraste des couleurs hérite probablement des bons réglages par "
        "défaut de Bootstrap 5, mais n'a pas été mesuré avec un outil dédié. Deux critères ne "
        "sont en revanche pas tenus : le graphique Plotly de la page Charts n'a aucun attribut "
        "alt ni aria-label, et aucun lien d'évitement (« Aller au contenu ») n'est présent dans "
        "templates/base.html."
    )
    doc.add_paragraph(
    ...
    add_figure(doc, "crop_login.png",
        "Figure 1 — Page de connexion réelle (accounts/login.html).")
    ...
    doc.add_paragraph(
        "Trois pages (Dashboard, Charts, Monitorage, cette dernière détaillée au rapport E3) "
    ...
    doc.add_paragraph(
        ...
        "Bloc4_app est ici auto-hébergé via docker-compose sur une seule machine, un choix "
    ...
    add_styled_table(doc,
        ["Fichier", "Rôle"],
        [
            ["accounts/views.py + forms.py", "Inscription (UserCreationForm), connexion (AuthenticationForm), déconnexion"],
            ["dashboard/views.py + services.py", "Vue d'ensemble, graphiques, monitorage de dérive ; DashboardService (appels data-api)"],
            ["dashboard/metrics.py", "Calcul de la dérive prédit/réalisé (cf. rapport E3, C11)"],
            ["forecast/views.py + forms.py + services.py", "Formulaire de classification, ForecastService (appels ml-api)"],
            ["crypto_app/settings.py + urls.py", "Configuration Django, routage racine"],
        ],
        col_widths=[8, 8],
    )
    ...
    add_figure(doc, "crop_forecast.png",
        ...
        "fonctionne de bout en bout et pas seulement en test mocké.")
    ...
    add_styled_table(doc,
        ...
        [
            ...
            ["A06 : Composants vulnérables ou obsolètes", "Dépendances figées et reproductibles (uv.lock), pas de scan automatisé de vulnérabilités en CI : une limite déjà identifiée côté ml-api en E3, toujours vraie ici."],
    ...
    doc.add_paragraph(
        "docker compose up -d, puis inscription, connexion, Dashboard (prix + prédiction stockée "
    ...
    doc.add_paragraph(
        ...
        "verte : un exemple concret de bug trouvé et corrigé en relisant le code avant "
    ...
    add_bullet(doc, "Robustesse asymétrique",
        ...
        "data-api produirait une erreur 500 non gérée plutôt qu'un message explicite.")
